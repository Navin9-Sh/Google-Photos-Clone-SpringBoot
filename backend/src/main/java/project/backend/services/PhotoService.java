...
public class PhotoService {
    ...
    public PageResponse<PhotoResponse> listPhotos(
            User user,
            PhotoStatus status,
            Pageable pageable
    ) {
        Page<Photo> page =
                photoRepository.findByUserIdAndStatus(
                        user.getId(),
                        status,
                        pageable
                );
    ...
    public PhotoResponse createDerivedPhoto(
        ...
        String thumbnailUrl = uploadResponse.thumbnailUrl()
                .orElseGet(() -> imageKitService.buildThumbnailUrlFromUrl(url));
    ...
    public PhotoResponse uploadPhoto(User user, MultipartFile file) {
        ...
        String url = uploadResponse.url()
                .orElseThrow(() -> new BadRequestException(
                        "ImageKit did not return a file url"
                ));
        ...
        if (filePath != null
                ...
                || photo.thumbnailUrl().isBlank())) {

            Photo savedPhoto = photoRepository.findByIdAndUserId(
                            photo.id(),
                            user.getId()
                    )
            ...
            savedPhoto.setThumbnailUrl(
                    imageKitService.buildThumbnailUrl(filePath)
            );
    ...
    public PhotoResponse createPhoto(User user, CreatePhotoRequest request) {
        if (photoRepository.existsByImagekitFileIdAndUserId(
                request.imagekitFileId(),
                user.getId()
        )) {
    ...
    public void restorePhotos(User user, List<UUID> photoIds) {
        List<Photo> photos =
                photoRepository.findByIdInAndUserId(photoIds, user.getId());
    ...
    private void updatePhotoStatus(
        ...
        for (Photo photo : photos) {
            if (requiredCurrentStatus != null
                    && photo.getStatus() != requiredCurrentStatus) {
                throw new BadRequestException(
                        "Photo must be " + requiredCurrentStatus.name().toLowerCase()
                );
            }
    ...
    private void validateUpload(MultipartFile file) {
        ...
        if (contentType == null
            ...
            throw new BadRequestException(
                    "Only image files are allowed (JPEG, PNG, WebP, GIF, HEIC, HEIF)"
            );
        }
    }
