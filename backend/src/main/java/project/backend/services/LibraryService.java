...
public class LibraryService {
    ...
    public StorageUsageResponse getStorageUsage(User user) {
        long usedBytes =
                photoRepository.sumActivePhotoBytesByUserId(user.getId());

        long photoCount =
                photoRepository.countByUserIdAndStatus(
                        user.getId(),
                        PhotoStatus.ACTIVE
                );
    ...
    public List<ImageKitAssetResponse> listImportableAssets(User user) {
        ...
        List<AssetListResponse> assets =
                imageKitService.listAssetsInFolder(
                        folder,
                        0,
                        100
                );

        List<ImageKitAssetResponse> responses =
                new ArrayList<>();
        ...
        for (AssetListResponse asset : assets) {
            ...
            responses.add(
                    new ImageKitAssetResponse(
                            ...
                            photoRepository.existsByImagekitFileIdAndUserId(
                                    fileId,
                                    user.getId()
                            )
                    )
    ...
    @Transactional
    public List<PhotoResponse> importAssets(
            User user,
            ImportPhotosRequest request
    ) {
        List<PhotoResponse> imported =
                new ArrayList<>();
